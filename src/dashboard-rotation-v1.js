...
async function rotationLatest(req,res){
  try{
    ...
    const assetViewBy=new Map(viewAssets.map(x=>[x.asset_class,x]));
    const sectorViewBy=new Map(viewSectors.map(x=>[`${x.asset_class||"STOCK"}:${x.sector||""}`,x]));
    const assets=(rawAssets.length?rawAssets:viewAssets).map(x=>Object.assign({},assetViewBy.get(x.asset_class)||{},x)).sort((a,b)=>(Number(a.rank)||99)-(Number(b.rank)||99));
    const sectors=(rawSectors.length?rawSectors:viewSectors).map(x=>Object.assign({},sectorViewBy.get(`${x.asset_class||"STOCK"}:${x.sector||""}`)||{},x)).sort((a,b)=>(Number(a.rank)||99)-(Number(b.rank)||99));
...
<script id="gn-rotation-script-v2">(function(){
...
function patchHtml(html){
  if(typeof html!=="string"||!html.includes("<title>GN PIVOT</title>")||html.includes("gn-rotation-script-v2"))return html;
