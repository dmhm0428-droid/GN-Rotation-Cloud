...
async function rotationLatest(req,res){
  try{
    const [summaryR,assetsR,sectorsR]=await Promise.all([
    ...
    res.json({ts:new Date().toISOString(),summary:summaryR.data||null,assets:assetsR.data||[],sectors:sectorsR.data||[]});
...
async function loadRotation(){try{const r=await fetch('/api/rotation/latest?t='+Date.now(),{cache:'no-store'});if(!r.ok)throw Error('HTTP '+r.status);const d=await r.json(),s=d.summary||{},rows=d.assets||[];$('rotationLeader').textContent='선두 '+(names[s.leader_asset]||s.leader_asset||'--');$('rotationSummary').textContent=s.rotation_summary&&s.rotation_summary!=='뚜렷한 자산군 회전 없음'?(names[s.rotation_from]||s.rotation_from)+' ↓ → '+(names[s.rotation_to]||s.rotation_to)+' ↑':'뚜렷한 자산군 이동 없음';$('rotationRows').innerHTML=rows.map(x=>'<div class="rotationRow"><b>'+esc(names[x.asset_class]||x.asset_class)+'</b><span>'+esc(x.rank)+(num(x.rank_change)>0?' ↑'+x.rank_change:num(x.rank_change)<0?' ↓'+Math.abs(x.rank_change):'')+'</span><span>'+f(x.rs_1d)+'</span><span>'+f(x.rs_5d)+'</span><span class="'+cls(x.flow_acceleration)+'">'+arrow(x.flow_acceleration)+' '+f(Math.abs(num(x.flow_acceleration)))+'</span><span class="'+cls(x.flow_acceleration)+'">'+esc(stages[x.flow_stage]||x.flow_stage||'--')+'</span></div>').join('');$('sectorRotation').innerHTML=(d.sectors||[]).slice(0,6).map(x=>'<div><b>'+esc(String(x.sector||'').replaceAll('_',' '))+'</b><small>순위 '+esc(x.rank)+' · RS1D '+f(x.rs_1d)+' · RS5D '+f(x.rs_5d)+'</small><small class="'+cls(x.flow_acceleration)+'">'+arrow(x.flow_acceleration)+' 가속 '+f(Math.abs(num(x.flow_acceleration)))+' · '+esc(stages[x.flow_stage]||x.flow_stage||'')+'</small></div>').join('')||'<div>섹터 데이터 없음</div>';}catch(e){if($('rotationSummary'))$('rotationSummary').textContent='자금회전 데이터 재조회 중';}}
