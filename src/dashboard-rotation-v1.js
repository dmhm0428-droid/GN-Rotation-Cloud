...
function closestBefore(rows,target){
  ...
  for(const r of rows||[]){const t=new Date(r.ts).getTime();if(!Number.isFinite(t)||t>target)continue;const d=target-t;if(d<bestDiff){best=r;bestDiff=d;}}
  return best;
}
function deriveAssetMomentum(latest,history){
  const by={};for(const r of history||[])(by[r.asset_class]||(by[r.asset_class]=[])).push(r);
  return (latest||[]).map(row=>{
    const rows=(by[row.asset_class]||[]).slice().sort((a,b)=>new Date(b.ts)-new Date(a.ts));
    const nowTs=new Date(row.ts).getTime(),cur=Number(row.flow_score);
    const d1=closestBefore(rows,nowTs-24*3600000),d5=closestBefore(rows,nowTs-5*24*3600000);
    const p1=Number(d1?.flow_score),p5=Number(d5?.flow_score);
    const rs1=Number.isFinite(cur)&&Number.isFinite(p1)?cur-p1:null;
    const rs5=Number.isFinite(cur)&&Number.isFinite(p5)?cur-p5:null;
    const accel=Number.isFinite(rs1)&&Number.isFinite(rs5)?rs1-(rs5/5):null;
    let stage=row.flow_stage;
    if(!stage&&Number.isFinite(accel))stage=accel>2?"STRENGTHENING":accel<-2?"WEAKENING":Number.isFinite(rs1)&&rs1>1?"LEADING":"NEUTRAL";
    return {...row,rs_1d:row.rs_1d??rs1,rs_5d:row.rs_5d??rs5,flow_acceleration:row.flow_acceleration??accel,flow_stage:stage||"DATA_WAIT",derived_rotation:row.rs_1d==null};
  });
}
async function rotationLatest(req,res){
  try{
    const since=new Date(Date.now()-6*24*3600000).toISOString();
    const [summaryR,viewAssetsR,viewSectorsR,rawAssets,rawSectors,histR]=await Promise.all([
      ...
      db.from("gn_asset_flow_scores").select("ts,asset_class,flow_score").gte("ts",since).order("ts",{ascending:false}).limit(5000)
    ]);
    if(summaryR.error)throw summaryR.error;if(histR.error)throw histR.error;
    const viewAssets=viewAssetsR.error?[]:(viewAssetsR.data||[]),viewSectors=viewSectorsR.error?[]:(viewSectorsR.data||[]);
    const mergedAssets=mergeByKey(viewAssets,rawAssets,r=>r?.asset_class||null).sort((a,b)=>(Number(a.rank)||99)-(Number(b.rank)||99));
    const assets=deriveAssetMomentum(mergedAssets,histR.data||[]);
    ...
    res.json({ts:new Date().toISOString(),summary:summaryR.data||null,assets,sectors,rotationDerived:assets.some(x=>x.derived_rotation)});
...
<script id="gn-rotation-script-v4">(function(){
