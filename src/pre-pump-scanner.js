...
async function enrichLatePumpRisk(rows,{fetchImpl=fetch,limit=DAILY_RISK_TOP_N}={}){
  ...
  const results=await Promise.allSettled(targets.map(async row=>{
    ...
    const risk=latePumpRisk(days);const score=Math.max(0,row.score-risk.latePumpPenalty);let state=row.state;
    if(risk.latePumpRisk&&state==="ENTRY")state="NO_CHASE";else if(score<70&&state==="ENTRY")state="SCOUT";
    return {...row,...risk,score:+score.toFixed(2),state};
  }));
...
async function scanPrePump({fetchImpl=fetch,sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms)),batchSize=BATCH_SIZE,batchDelayMs=BATCH_DELAY_MS,derivatives={}}={}){
  ...
  return enriched.filter(row=>row.score>=50&&(row.state==="SCOUT"||row.state==="ENTRY")).slice(0,3);
}

module.exports={calculate15mStructure,calculateMetrics,calculateObvDirection,classify1hStructure,derivativeScore,enrichLatePumpRisk,fetchJson,highChasePenalty,highDistance1h,latePumpRisk,rankCandidates,resistanceScore,rsi14,scanPrePump,scoreCandidates,stateOf,sumTurnover};
