...
const STATIC_ROWS=HEADINGS.map(([,label])=>`<div style="padding:10px 0;border-top:1px solid #202a33"><b>${label}</b></div>`).join("");
const PANEL=`<section id="gnPolicyDetected" style="margin:14px 0;background:#10151b;border:1px solid #293540;border-radius:16px;padding:15px"><div style="font-size:14px;font-weight:900;margin-bottom:9px">큰그림 정책 대시보드</div><div id="gnPolicyDetectedRows">${STATIC_ROWS}</div></section>`;
...
async function load(){render(null);try{var r=await fetch('/api/alerts?t='+Date.now(),{cache:'no-store'});if(r.status===401){location.href='/login';return}if(!r.ok){render(null);return}var a=await r.json(),list=Array.isArray(a)?a:(Array.isArray(a&&a.alerts)?a.alerts:[]),row=list.find(function(x){return x.level==='큰그림 정책'}),payload=null;try{payload=row?JSON.parse(row.message):null}catch(e){}render(payload)}catch(e){render(null)}}
