...
function auth(env=process.env){if(!env.UPBIT_ACCESS_KEY||!env.UPBIT_SECRET_KEY)return null;return {access:env.UPBIT_ACCESS_KEY,secret:env.UPBIT_SECRET_KEY};}
async function balances({env=process.env,fetchImpl=fetch}={}){const a=auth(env);if(!a)return {enabled:false,exchange:"upbit",balances:[]};const token=jwtHS256({access_key:a.access,nonce:crypto.randomUUID()},a.secret);const rows=await requestJson(`${BASE}/v1/accounts`,{headers:{Authorization:`Bearer ${token}`},fetchImpl});return {enabled:true,exchange:"upbit",balances:(rows||[]).map(r=>normalizeBalance("upbit",{asset:r.currency,free:r.balance,locked:r.locked,avgPrice:r.avg_buy_price}))};}
