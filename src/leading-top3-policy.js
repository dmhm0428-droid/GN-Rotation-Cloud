...
function passesTop3Gate(row){
  ...
  const core=row?.preExpansionEligible===true&&lead&&repeat>=2&&flow.available&&flow.score>=60&&globalConfirmed&&num(row?.validationConfidence)>=90;
  if(!core)return false;
  // 5AI is an execution gate, not a ranking bonus.  An ENTRY label must prove it.
