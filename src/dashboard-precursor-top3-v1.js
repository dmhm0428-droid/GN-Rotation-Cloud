...
const SCRIPT=`<script id="gn-precursor-top3-v1">(function(){
  ...
  function render(){if(rendering||!payload)return;var box=document.getElementById('top3');if(!box)return;var rows=(payload.cryptoRadar||[]).slice(0,3);rendering=true;try{box.innerHTML=payload.precursorStale?'<div class="empty">전조 데이터 지연 · 신규 진입 금지</div>':rows.length?rows.map(card).join(''):'<div class="empty">현재 유효 전조 TOP3 없음</div>';box.dataset.gnPrecursor='1';var pill=document.getElementById('cryptoState');if(pill){var c3=rows.filter(function(x){return x.precursorStage==='CONFIRMED_REPEAT_3'}).length,c2=rows.filter(function(x){return x.precursorStage==='REPEAT_2'}).length,en=rows.filter(function(x){return x.entryAllowed===true&&String(x.status||'').toUpperCase()==='ENTRY'}).length;pill.textContent='반복확정 '+c3+' · 반복2 '+c2+' · 신규 '+Math.max(0,rows.length-c3-c2)+' · ENTRY '+en}var sec=box.closest('section');if(sec){var t=sec.querySelector('.sectionTitle'),s=sec.querySelector('.sub');if(t)t.textContent='크립토 전조 TOP3 · 반복 우선';if(s)s.textContent='전조 → 반복 TOP3 → 해외현물·파생 확장검증 → 사후검증';}}finally{setTimeout(function(){rendering=false},0)}}
  ...
  var obs=new MutationObserver(function(m){if(rendering||!payload)return;for(var i=0;i<m.length;i++){var target=m[i].target;if(target&&((target.id==='top3')||(target.closest&&target.closest('#top3')))){var box=document.getElementById('top3');if(box&&box.dataset.gnPrecursor!=='1')setTimeout(render,0);break}}});
