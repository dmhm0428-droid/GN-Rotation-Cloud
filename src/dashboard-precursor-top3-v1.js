...
const SCRIPT=`<script id="gn-precursor-top3-v1">(function(){
  ...
  function check(label,v){return '<span class="'+(v===true?'precursorOk':'precursorNo')+'">'+label+' '+(v===true?'✓':'제외')+'</span>'}
  ...
  function card(r,i){var p=r.persistence||{},e=r.expansion||{},post=r.postValidation||{};var first=r.firstDetectedPrice!=null?nf(r.firstDetectedPrice)+'원':'--';var now=r.currentPrice!=null?nf(r.currentPrice)+'원':'--';var rise=r.riseSinceFirstPct==null?'':(' · 최초후 '+pct(r.riseSinceFirstPct));var observe=r.observationOnly!==false?' · 관찰신호, ENTRY 아님':'';var rel='15m BTC대비 '+pct(r.relative15mVsBtc)+' · ETH대비 '+pct(r.relative15mVsEth)+' · 60m BTC대비 '+pct(r.relative60mVsBtc)+' · ETH대비 '+pct(r.relative60mVsEth);return '<article class="pick" data-gn-precursor-card="1"><div class="pickRank">'+(i+1)+'</div><div class="pickMain"><div class="pickName">'+esc(String(r.market||'').replace('KRW-',''))+' <span class="precursorBadge '+stageClass(r)+'">'+esc(r.stageLabel||'신규 전조')+'</span></div><div class="pickPrice">현재 '+now+' · 최초 '+tm(r.firstDetectedAt)+' '+first+rise+'</div><div class="precursorLine">'+check('BTC·ETH 독립',r.btcEthIndependent)+' · 독립점수 '+nf(r.independenceScore)+' · 당일발동 '+nf(r.sameDayActivationScore)+' · '+esc(r.activationWindow||'1-6H')+' · 당일청산</div><div class="precursorLine">'+rel+'</div><div class="precursorLine">반복 '+nf(r.repeatCount)+'회 · 30분 TOP3 '+nf(p.top3Count30m)+'회 · 연속 '+nf(p.consecutiveTop3)+'회 · 6시간 TOP6 '+nf(p.top6Count6h)+'회</div><div class="precursorLine">확장 '+(e.score==null?'--':nf(e.score))+' · '+check('해외현물',e.globalSpotOk)+' · '+check('파생',e.derivativesOk)+' · '+check('온체인',e.onchainOk)+' · MA '+(r.maScore==null?'--':nf(r.maScore))+'</div><div class="precursorLine">'+postLine(post)+'</div><div class="precursorLearn">'+learnLine(r.validationStats)+'</div></div><div><div class="precursorAction '+actionClass(r)+'">'+esc(r.precursorAction||'전조감시')+'</div><div class="precursorWhy">'+esc(e.thesis||r.details?.decision_reason||'독립강도·당일발동 검증 중')+observe+'</div></div></article>'}
  function render(){if(rendering||!payload)return;var box=document.getElementById('top3');if(!box)return;var rows=(payload.cryptoRadar||[]).filter(function(x){return x.btcEthIndependent===true}).slice(0,3);rendering=true;try{box.innerHTML=payload.precursorStale?'<div class="empty">전조 데이터 지연 · 신규 진입 금지</div>':rows.length?rows.map(card).join(''):'<div class="empty">BTC·ETH 독립강세 조건 충족 종목 없음 · 오늘 ENTRY 없음</div>';var pill=document.getElementById('cryptoState');if(pill){var ready=rows.filter(function(x){return x.sameDayReady===true}).length,en=rows.filter(function(x){return Number(x.entrySlot)===1&&String(x.status||'').toUpperCase()==='ENTRY'}).length;pill.textContent='독립강세 '+rows.length+' · 당일발동 '+ready+' · ENTRY '+en+' / 최대1'}var sec=box.closest('section');if(sec){var t=sec.querySelector('.sectionTitle'),s=sec.querySelector('.sub');if(t)t.textContent='크립토 독립강세 TOP3 · ENTRY 0~1';if(s)s.textContent='BTC·ETH 비동조 상대강도 → 1~6H 당일발동 → 해외·파생 검증 → 당일청산';}}finally{setTimeout(function(){rendering=false},0)}}
