...
app.get("/api/market/live",async(req,res)=>{
  try{
    ...
    const [{data:btcRows,error:btcErr},{data:savedMarket,error:savedErr}]=await Promise.all([
...
function hero(m){const d=m?.decision;if(!d)return '<div class="bad">시장 전체 데이터 수집 실패</div>';return '<div class="muted">지금 행동</div><div class="action">'+d.action+'</div><div class="marketscore">'+d.score+'</div><div>'+d.regime+'</div><div>'+((d.reasons||[]).map(x=>'<span class="pill">'+x+'</span>').join(''))+'</div>';}
...
async function loadAll(){try{const [market,latest,alerts,prePump]=await Promise.all([safe('/api/market/live'),safe('/api/latest'),safe('/api/alerts'),safe('/api/pre-pump/latest').catch(()=>[])]);document.getElementById('hero').innerHTML=hero(market);document.getElementById('marketMetrics').innerHTML=metrics(market);document.getElementById('cards').innerHTML=coinCards(latest);document.getElementById('prePump').innerHTML=prePumpTable(prePump);document.getElementById('movers').innerHTML=movers(market);document.getElementById('updated').textContent='실시간 시장 '+new Date(market.ts).toLocaleString()+' · 자동 갱신 60초';document.getElementById('alerts').innerHTML=(alerts||[]).slice(0,12).map(a=>'<tr><td>'+new Date(a.ts).toLocaleString()+'</td><td>'+a.coin+'</td><td>'+(a.stage||a.level)+'</td><td>'+a.message+'</td></tr>').join('');}catch(e){document.getElementById('updated').textContent='오류: '+e.message;}}loadAll();setInterval(loadAll,60000);
</script></body></html>`));
