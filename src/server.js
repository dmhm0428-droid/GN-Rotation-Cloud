...
app.get("/",(req,res)=>res.type("html").send(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>GN PIVOT</title><style>:root{color-scheme:dark}*{box-sizing:border-box}body{font-family:system-ui,-apple-system,sans-serif;margin:0;background:#080a0d;color:#eef2f6}.wrap{max-width:720px;margin:auto;padding:14px 14px 40px}.top{display:flex;justify-content:space-between;align-items:center;gap:10px}.muted{color:#99a5b1;font-size:12px}.topActions{display:flex;gap:8px;align-items:center}.logout{color:#b7c1cb;text-decoration:none;border:1px solid #3a4654;padding:8px 10px;border-radius:9px;font-size:12px}button{background:#202833;color:white;border:1px solid #3a4654;padding:8px 12px;border-radius:9px}h1{font-size:21px;margin:8px 0}h2{font-size:15px;margin:20px 2px 9px}.hero,.card{background:#12171d;border:1px solid #27313b;border-radius:16px;padding:16px}.hero{margin:12px 0;border-color:#364352}.eyebrow{font-size:12px;color:#99a5b1;margin-bottom:4px}.action{font-size:38px;font-weight:950;letter-spacing:-1.5px}.subaction{margin-top:4px;font-size:14px;color:#b7c1cb}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}.coin{min-width:0}.coinName{font-size:14px;font-weight:800;color:#c4ced8}.coinAction{font-size:23px;font-weight:950;margin:7px 0;line-height:1.05}.price{font-size:13px;font-weight:800}.good{color:#55d98b}.warn{color:#ffd166}.bad{color:#ff6b6b}.neutral{color:#b7c1cb}.top3{display:grid;gap:8px}.pick{display:grid;grid-template-columns:34px 1fr auto;align-items:center;gap:10px;background:#12171d;border:1px solid #27313b;border-radius:14px;padding:13px}.rank{width:28px;height:28px;border-radius:50%;display:grid;place-items:center;background:#202833;font-weight:900}.pickName{font-weight:900}.pickMeta{font-size:12px;color:#99a5b1;margin-top:2px}.pickAction{font-size:17px;font-weight:950;text-align:right}.empty{padding:16px;color:#99a5b1;background:#12171d;border:1px solid #27313b;border-radius:14px}.foot{margin-top:18px;text-align:center;color:#687582;font-size:11px}@media(max-width:560px){.grid{grid-template-columns:1fr}.card{display:grid;grid-template-columns:72px 1fr auto;align-items:center;gap:8px;padding:13px}.coinAction{margin:0;font-size:22px}.price{text-align:right}.action{font-size:36px}}</style></head><body><div class="wrap"><div class="top"><div><h1>GN PIVOT</h1><div class="muted" id="updated">불러오는 중…</div></div><div class="topActions"><button onclick="loadAll()">새로고침</button><a class="logout" href="/logout">로그아웃</a></div></div><div class="hero" id="hero"></div><h2>BTC · ETH · SOL</h2><div class="grid" id="majors"></div><h2>지금 볼 TOP3</h2><div class="top3" id="top3"></div><div class="foot">내부에서는 시장·수급·파생·추세·누적수급·매물대를 계속 계산합니다.</div></div><script>
...
function gnAction(row){const s=String(row?.action||'').trim();if(s==='매도')return {text:'팔아라',cls:'bad'};if(s==='매도준비')return {text:'팔 준비',cls:'warn'};if(s==='보유점검')return {text:'지켜봐',cls:'warn'};if(s==='돌파보유'||s==='보유')return {text:'들고가',cls:'good'};if(s==='진입유지'||s==='진입')return {text:'사라',cls:'good'};if(s==='추격금지')return {text:'사지 마라',cls:'bad'};return simpleAction(row?.status,row?.score);}
function marketHero(m){const d=m?.decision;if(!d)return '<div class="action bad">기다려</div><div class="subaction">시장 데이터 확인 중</div>';const a=simpleAction(d.action,d.score);return '<div class="eyebrow">지금 시장</div><div class="action '+a.cls+'">'+a.text+'</div><div class="subaction">'+(d.regime||'')+'</div>';}
...
function top3Cards(rows){const top=(rows||[]).slice(0,3);if(!top.length)return '<div class="empty">지금은 신규 진입 후보 없음</div>';return top.map((r,i)=>{const a=gnAction(r);const repeat=r.repeatCount>1?' · 연속 '+r.repeatCount+'회':'';const chase=r.newEntryAllowed===false?' · 신규추격 금지':'';const px=r.krwPrice!=null?' · '+n(r.krwPrice)+'원':'';return '<div class="pick"><div class="rank">'+(i+1)+'</div><div><div class="pickName">'+String(r.market||'').replace('KRW-','')+'</div><div class="pickMeta">'+(r.status||'')+repeat+chase+px+'</div></div><div class="pickAction '+a.cls+'">'+a.text+'</div></div>';}).join('');}
