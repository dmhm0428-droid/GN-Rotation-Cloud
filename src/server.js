...
function top3Cards(rows){const top=(rows||[]).slice(0,3);if(!top.length)return '<div class="empty">지금은 신규 진입 후보 없음</div>';return top.map((r,i)=>{const a=simpleAction(r.status,r.score);return '<div class="pick"><div class="rank">'+(i+1)+'</div><div><div class="pickName">'+String(r.market||'').replace('KRW-','')+'</div><div class="pickMeta">'+(r.status||'')+'</div></div><div class="pickAction '+a.cls+'">'+a.text+'</div></div>';}).join('');}
