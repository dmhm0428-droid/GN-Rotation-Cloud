...
function portfolioCards(p){const rows=p?.exchanges||[];if(!rows.length)return '<div class="card bad">거래소 정보를 불러오지 못했습니다.</div>';return rows.map(ex=>{const title='<div class="exchangeName">'+ex.exchange+'</div>';if(!ex.enabled)return '<div class="card">'+title+'<div class="muted">API 연결 안 됨</div></div>';if(ex.error)return '<div class="card">'+title+'<div class="bad">잔고 조회 실패</div><div class="muted">'+ex.error+'</div></div>';const positions=(ex.positions||[]);if(!positions.length)return '<div class="card">'+title+'<div class="muted">보유 잔고 없음</div></div>';const body=positions.map(x=>'<div class="position"><div><b>'+x.asset+'</b><div class="muted">수량 '+amount(x.total)+'</div></div><div class="positionValue"><b>'+amount(x.valueQuote,2)+' '+x.quote+'</b><div class="muted">평단 '+amount(x.avgPrice,2)+'</div></div></div>').join('');return '<div class="card">'+title+body+'</div>';}).join('');}
function activeSignals(alerts){
  const active=(alerts||[]).filter(a=>a.active);
  const seen=new Set();
  const latestByCoin=active.filter(a=>{const key=a.coin||'MARKET';if(seen.has(key))return false;seen.add(key);return true;}).slice(0,12);
  if(!latestByCoin.length)return '<tr><td colspan="5" class="signalEmpty">현재 활성 신호 없음 · 지난 신호는 매수 근거로 사용하지 않음</td></tr>';
  return latestByCoin.map(a=>'<tr><td><span class="signalActive">활성</span></td><td>'+new Date(a.ts).toLocaleString()+'</td><td>'+a.coin+'</td><td>'+(a.stage||a.level||'')+'</td><td>'+a.message+'<div class="muted">'+a.age_minutes+'분 전 발생</div></td></tr>').join('');
}
async function loadAll(){try{const [market,latest,alerts,prePump,portfolio]=await Promise.all([safe('/api/market/live'),safe('/api/latest'),safe('/api/alerts'),safe('/api/pre-pump/latest').catch(()=>[]),safe('/api/portfolio').catch(e=>({exchanges:[],error:e.message}))]);document.getElementById('hero').innerHTML=hero(market);document.getElementById('aiConsensus').innerHTML=aiCards(market.ai);document.getElementById('marketMetrics').innerHTML=metrics(market);document.getElementById('cards').innerHTML=coinCards(latest);document.getElementById('prePump').innerHTML=prePumpTable(prePump);document.getElementById('movers').innerHTML=movers(market);document.getElementById('portfolio').innerHTML=portfolioCards(portfolio);document.getElementById('updated').textContent='실시간 시장 '+new Date(market.ts).toLocaleString()+' · 자동 갱신 60초';document.getElementById('alerts').innerHTML=activeSignals(alerts);}catch(e){document.getElementById('updated').textContent='오류: '+e.message;}}loadAll();setInterval(loadAll,60000);
</script></body></html>`));
